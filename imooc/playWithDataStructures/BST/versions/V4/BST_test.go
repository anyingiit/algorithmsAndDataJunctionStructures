...
func TestBST_Push(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(actuallyResponse) != len(tt.expectPushResponse) {
			t.Errorf("test BST Enqueue failed: push response len check failed, actually %v expect %v. input %v expectPushResponse %v actuallyResponse %v", len(actuallyResponse), len(tt.expectPushResponse), tt.input, tt.expectPushResponse, actuallyResponse)
		}

		for i, e := range tt.expectPushResponse {
			if actuallyResponse[i] != e {
				t.Errorf("test BST Enqueue failed: push response check failed, actually %v expect %v. input %v expectPushResponse %v actuallyResponse %v", actuallyResponse[i], e, tt.input, tt.expectPushResponse, actuallyResponse)
			}
		}
		...
		for !bst.IsEmpty() {
			...
			if err != nil {
				t.Errorf("test BST Enqueue failed: RemoveMin failed %s, input %v", err.Error(), tt.input)
			}
			actuallyOutput = append(actuallyOutput, min)
		}

		if len(actuallyOutput) != len(tt.expectOutput) {
			t.Errorf("test BST Enqueue failed: check expect output len failed, actually %d expect %d. input %v expectOutput %v actuallyOutput %v", len(actuallyOutput), len(tt.expectOutput), tt.input, tt.expectOutput, actuallyOutput)
		}

		for i, e := range tt.expectOutput {
			if actuallyOutput[i] != e {
				t.Errorf("test BST Enqueue failed: check expect output element failed, actually %d expect %d. input %v expectOutput %v actuallyOutput %v", actuallyOutput[i], e, tt.input, tt.expectOutput, actuallyOutput)
			}
		}
	}
}
